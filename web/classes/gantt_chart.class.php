	...
	class GanttChart extends Database {
		...
		function getActivities()
		{
			...
			$sql = "SELECT projects.`project_name`, gantt_chart.`date_added`, gantt_chart.`id`, gantt_chart.`project_id`, 
						gantt_chart.`activity`, `amount`, gantt_chart.`date_start`, gantt_chart.`date_end`
					FROM projects
		...
		function getProjectActivities($project_id)
		{
			...
			if (!empty($errors)) {
				...
				$sql2 = "SELECT projects.`project_name`, gantt_chart.`date_added`, 
                        gantt_chart.`activity`, `amount`, gantt_chart.`date_start`, gantt_chart.`date_end`, gantt_chart.`id`
						FROM projects
						INNER JOIN gantt_chart ON gantt_chart.project_id = projects.id 
		...
		function addActivity($date_added, $activity, $amount, $date_start, $date_end, $project_id)
		{
			...
			if (!empty($errors)) {
				...
				$sql = "INSERT INTO gantt_chart (date_added, activity, amount, date_start, date_end, project_id) VALUES (now(), '$activity', $amount,'$date_start', '$date_end', '$project_id')";
		...
		function getProjectChart($project_id)
        {
            $data = array();
            $sql = "SELECT gantt_chart_resource.*, gantt_chart.date_start, gantt_chart.date_end, resources.id as resources_id, resources.`name` AS resources_name FROM gantt_chart_resource, gantt_chart, resources WHERE gantt_chart_resource.gantt_chart_id = gantt_chart.id AND resources.`id` = gantt_chart_resource.`resource_id` AND gantt_chart_resource.project_id='$project_id' ORDER BY resources.id";
            // We are now able to access this method because we extended the "Database" class.
            $result = $this->getSQLResult($sql);
            $data[] = $result;

            $sql = "SELECT gantt_chart_resource1.*, gantt_chart.date_start, gantt_chart.date_end, equipment.id as equipment_id, equipment.`name` AS equipment_name FROM gantt_chart_resource1, gantt_chart, equipment WHERE gantt_chart_resource1.gantt_chart_id = gantt_chart.id AND equipment.`id` = gantt_chart_resource1.`resource_id` AND gantt_chart_resource1.project_id='$project_id' ORDER BY equipment.id";
            // We are now able to access this method because we extended the "Database" class.
            $result = $this->getSQLResult($sql);
            $data[] = $result;

            $sql = "SELECT * FROM image WHERE projects_id='$project_id'";
            // We are now able to access this method because we extended the "Database" class.
            $result = $this->getSQLResult($sql);
            $data[] = $result;
