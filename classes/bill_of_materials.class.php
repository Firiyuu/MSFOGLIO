<?php 
	...
	class BillofMaterials extends Database {
		...
		{
			$sql = "SELECT projects.`project_name`, bill_of_materials.`item`, bill_of_materials.`quantity`, bill_of_materials.`amount`, bill_of_materials.`total`, bill_of_materials.`id`, bill_of_materials.`description`, bill_of_materials.`unit`
			FROM projects
		...
		function getBillofMaterialsActivities($project_id)
		{
			...
			if (!empty($errors)) {
				...
				$sql2 = "SELECT projects.`project_name`, bill_of_materials.`item`, 
							bill_of_materials.`quantity`, bill_of_materials.`amount`, bill_of_materials.`total`, bill_of_materials.`id`, bill_of_materials.`paid`, bill_of_materials.`date_in`, bill_of_materials.`unit`, bill_of_materials.`description`
						FROM projects
						INNER JOIN bill_of_materials ON bill_of_materials.project_id = projects.id 
		...
		function addBillofMaterial($project_id, $item, $quantity, $amount, $unit, $description, $total)
		{
			...
			if (!empty($errors)) {
				...
				$sql = "
					INSERT INTO bill_of_materials (project_id, item, quantity, amount, total, unit, description) 
					VALUES
					  ('$project_id', '$item', '$quantity', '$amount', '$total', '$unit', '$description');
				";
		...
		function updateBillofMaterial($id, $project_id, $item, $quantity, $amount, $unit, $description, $total)
		{
			$errors = [];
			...
			if (!empty($errors)) {
				...
				$sql = "UPDATE bill_of_materials SET project_id='$project_id', item='$item', quantity='$quantity', amount='$amount', total='$total', unit='$unit', description='$description'WHERE id='$id'";
